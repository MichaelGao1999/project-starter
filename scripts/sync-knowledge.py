...
def merge_decisions(local_text: str, remote_entries: list[dict], repo_name: str) -> str:
    ...
    for e in remote_entries:
        ...
        for existing in existing_titles:
            ...
            if similarity(title_clean, existing_clean) > 0.75:
                log(f"  跳过相似决策: {title_clean}（与「{existing_clean}」相似度 {similarity(title_clean, existing_clean):.0%}）")
                dup = True
                break
        if dup:
            continue
...
def merge_lessons(local_text: str, remote_entries: list[dict], repo_name: str) -> str:
    ...
    for e in remote_entries:
        ...
        for existing in existing_descs:
            existing_clean = re.sub(r"\[来源:.+?\]", "", existing).strip()
            if similarity(desc, existing_clean) > 0.75:
                skipped_similar += 1
                dup = True
                break
        if dup:
...
def merge_troubleshooting(local_text: str, remote_entries: list[dict], repo_name: str) -> str:
    ...
    for e in remote_entries:
        ...
        for existing in existing_keywords:
            existing_clean = re.sub(r"\[来源:.+?\]", "", existing).strip()
            if similarity(keyword, existing_clean) > 0.75:
                log(f"  跳过相似问题: {keyword}（与「{existing_clean}」相似度 {similarity(keyword, existing_clean):.0%}）")
                dup = True
                break
        if dup:
            continue
